...
export function setupHealth({ api, getConfig }) {
  ...
  $('network-check').onclick = async () => {
    ...
    try {
      $('network-result').textContent = (await api('/audio/health/network', {})).detail;
    } catch {
      $('network-result').textContent = '检查未完成，请确认本地服务仍在运行。';
    } finally {
      running = false;
      $('network-check').disabled = false;
    }
  };
  return { refresh };
}
