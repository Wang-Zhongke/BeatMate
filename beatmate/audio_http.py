...
def route(h, audio, body):
    ...
    if h.command == "GET" and path in ("/", "/audio"):
        return h.send(
            200,
            Path(__file__).with_name("audio.html").read_bytes(),
    ...
    if h.command == "GET" and path == "/audio/library/revision":
        return h.send(200, audio.catalog_revision())
    ...
    static = {
        "/audio/ui/studio.css": ("studio.css", "text/css; charset=utf-8"),
        "/audio/ui/studio.js": ("studio.js", "text/javascript; charset=utf-8"),
        "/audio/ui/cover.png": ("cover.png", "image/png"),
    }
    ...
    if h.command == "GET" and path == "/audio/health":
        return h.send(200, audio.health.report(audio))
    if h.command == "POST" and path == "/audio/health/network":
        fields(body, [])
        return h.send(200, audio.health.network(audio))
    if h.command == "GET" and path == "/audio/config":
        return h.send(200, audio.config())
    if h.command == "GET" and path == "/audio/budget":
        return h.send(200, audio.budget())
    ...
    if h.command == "GET" and path == "/audio/history":
        return h.send(200, audio.history())
    if h.command == "POST" and path == "/audio/analyze-lyrics":
        fields(
            body,
            ["raw_text", "lyrics", "constraints", "request_id"],
            ["raw_text", "lyrics", "request_id"],
        )
    ...
    m = re.fullmatch(
        r"/audio/stems/([a-f0-9]{32})/midi(?:/([a-f0-9]{32}))?/download", path
    )
    if m and h.command == "GET":
        ...
        h.send_header(
            "Content-Disposition",
            f'attachment; filename="beatmate-midi-{aid}-{fid or "all"}.{suffix}"',
        )
        h.end_headers()
        h.wfile.write(data)
        return
    ...
    if m:
        if h.command == "GET" and not m[2]:
            return h.send(200, audio.get(m[1]))
    ...
    if m:
        if h.command == "GET":
            return h.send(200, audio.notes(m[1]))
    ...
    if m and h.command == "GET":
        with audio.export_bundle(m[1]) as (stream, name):
            ...
            h.send_header(
                "Content-Disposition",
                "attachment; filename=beatmate-export.zip; filename*=UTF-8''"
                + quote(name, safe=""),
            )
            h.send_header("X-Content-Type-Options", "nosniff")
            h.end_headers()
            shutil.copyfileobj(stream, h.wfile)
        return
    ...
    if m and h.command == "GET":
        ...
        if range_header:
            ...
            if not match:
                return h.send(416, {"error": "Unsupported byte range"})
            start = int(match[1])
            end = min(int(match[2]) if match[2] else end, end)
            if start > end:
                return h.send(416, {"error": "Invalid byte range"})
            status = 206
        mime = {
            "wav": "audio/wav",
            "mp3": "audio/mpeg",
        ...
        if m[2]:
            h.send_header(
                "Content-Disposition",
                f'attachment; filename="beatmate-{a.get("track_type", "original")}-{a["id"]}.{a["format"]}"',
            )
        h.end_headers()
        h.wfile.write(data[start : end + 1])
        return
