...
class Planner(Protocol):
    ...
    def plan_edit(self, text: str, track_id: str, start_bar: int, end_bar: int) -> dict: ...
...
class MockPlanner:
    ...
    def parse_intent(self, text):
        ...
        if 'trap' in text:
            ...
            data['style'] = 'boom_bap'
        for field, pattern in [('bpm', r'(\d+)\s*bpm'), ('bars', r'(\d+)\s*(?:bars?|小节)'), ('seed', r'seed\s*[=:]?\s*(\d+)')]:
        ...
        return dict(spec=spec.to_dict(), planner='mock', assumptions=[
    ...
    def plan_edit(self, text, track_id, start_bar, end_bar):
        ...
        if any(x in text for x in ('mute', '静音', '删除')):
            ...
            op, value = 'velocity', (-10 if '轻' in text else 10) if value is None else value
        elif any(x in text for x in ('transpose', '移调', '升高', '降低')):
            op, value = 'transpose', 12 if value is None else value
            if '降低' in text:
                value = -abs(value)
        else:
            raise ValueError('Mock 无法解析此编辑；请指定加密16、力度-10、移调+12或静音')
