...
def main():
    parser = argparse.ArgumentParser(
        description="BeatMate local native-MIDI agent (permanent Mock mode)"
    )
    ...
    creative.add_argument(
        "--planner", choices=["mock", "deepseek", "openai"], default=None
    )
    creative.add_argument("--reference-project")
    creative.add_argument("--reference-version")
    creative.add_argument(
        "--output", required=True, help="New artifact directory; never overwritten"
    )
    planning = sub.add_parser(
        "plan-edit", help="Plan against trusted saved state; never commits"
    )
    ...
    apply = sub.add_parser(
        "apply-edit", help="Explicitly commit a saved ready proposal"
    )
    apply.add_argument("--project", required=True)
    apply.add_argument("--proposal", required=True)
    apply.add_argument("--output", required=True)
    creative_demo = sub.add_parser(
        "creative-demo", help="Offline A/B/C; isolated temporary database"
    )
    ...
    selected = (
        (
            ...
            else os.environ.get("BEATMATE_PLANNER", "mock")
        )
        if runtime
        else "mock"
    )
    ...
    if args.command in ("creative", "plan-edit", "apply-edit"):
        ...
        if args.command == "creative":
            v = service.create_creative_project(
                args.text,
                reference_project_id=args.reference_project,
