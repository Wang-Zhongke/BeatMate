...
def plan_project_edit(planner, version, text, track_id=None, start_bar=None, end_bar=None, protected_tracks=()):
    from .service import protection
    lower = checked_text(text)
    ids = [t['id'] for t in version['tracks']]
    protected = protection(version['protected_tracks'] + protection(protected_tracks,ids),ids)
    if re.search(r'(鼓和贝斯|鼓和bass|鼓与贝斯).*(不要动|不变)', lower):
        protected = sorted(set(protected) | (set(ids)&{'kick','snare','hihat','bass'}))
    context = dict(base_version=version['version_id'],protected_tracks=protected,
                   project_state=dict(track_ids=ids,bars=version['spec']['bars'],bpm=version['spec']['bpm']))

    def suggestion(reason, proposed=None):
        return dict(status='suggestion',reason=reason,suggested_scope=proposed,plan=None,**context)

    if any(word in lower for word in ('温暖','空灵','warmer','ethereal')):
        return suggestion('当前只能建议音色/混响等制作方向，不能用移调或降低力度冒充温暖/空灵。')
    ...
    if not explicit_scope:
        ...
        if '后半段' in text and track_id:
            start_bar,end_bar=version['spec']['bars']//2+1,version['spec']['bars']
        elif track_id:
    ...
    if track_id is None or start_bar is None or end_bar is None:
        proposed=dict(track_id='melody',start_bar=version['spec']['bars']//2+1,end_bar=version['spec']['bars']) if 'melody' in ids else None
    ...
    thinning = any(word in lower for word in ('克制','减密','稀疏','thin_notes','sparser'))
    ...
    if not thinning and not any(word in lower for word in ('velocity','力度','transpose','移调','升高','降低','mute','静音','删除','density','密度','加密')):
    ...
    if isinstance(planner,MockPlanner):
        ...
        allowed=['thin_notes'] if thinning else ['velocity','transpose','density','mute']
        result=planner._complete(text,
            ...
            json.dumps(dict(**context,track_id=track_id,start_bar=start_bar,end_bar=end_bar)),
            schema(dict(operation=dict(type='string',enum=allowed),value=dict(type='integer'))))
        try:
    ...
    return dict(status='ready',plan=plan.to_dict(),scope_source='caller' if explicit_scope else 'resolved_from_text_and_project',
                interpretation='更克制在本轮仅解释为减少melody音符，保持锚点；不保证情绪效果。' if thinning else '执行显式音符操作。',**context)
