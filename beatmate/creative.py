...
class CreativeEditPlan(EditPlan):
    def __post_init__(self):
        ...
        limits = {'velocity': (-126, 126), 'transpose': (-24, 24), 'mute': (0, 0), 'thin_notes': (50, 50)}
...
def extract_brief(text):
    ...
    explicit = {k: [] for k in ('emotions', 'scene', 'expression', 'references', 'purpose', 'prohibitions', 'other')}
    vocabulary = {
        'emotions': ('伤感', 'emo', '孤独', '想念', '不甘', '怀旧', '温柔', '开心', 'sad', 'nostalgic'),
    ...
    for match in re.finditer(r'(?<![a-z])([a-g]#?)\s*(minor|major|小调|大调)', text, re.I):
        _put(constraints, sources, 'key', match[1].upper(), match[0])
        _put(constraints, sources, 'mode', 'minor' if match[2].lower() in ('minor','小调') else 'major', match[0])
    for term, value in [('小调','minor'), ('大调','major')]:
        if term in text:
            _put(constraints, sources, 'mode', value, term)
    for term, value in [('boom bap','boom_bap'), ('boom_bap','boom_bap'), ('trap','trap')]:
        if term in text.lower() and not re.search(r'(不要|避免|no)\s*'+re.escape(term),text,re.I):
            _put(constraints, sources, 'style', value, term)
    exact = {'分解和弦': ('chord_style','arpeggio'), '柱式和弦': ('chord_style','block'),
             '不要主旋律': ('melody_enabled',False), '无主旋律': ('melody_enabled',False),
             '旋律稀疏': ('melody_density','sparse'), '旋律适中': ('melody_density','moderate'),
             '鼓组稀疏': ('drum_density','sparse'), '鼓组适中': ('drum_density','moderate')}
    for quote, (name, value) in exact.items():
        if quote in text:
            if not quote.startswith(('不要', '无')) and re.search(r'(不要|不想|避免|别)\s*'+re.escape(quote),text):
                raise ValueError('Negated arrangement phrase needs an explicit supported alternative; not treated as a positive request')
    ...
    brief.limitations = ['情绪与共鸣需人工试听，不提供共鸣评分。',
        '音色提示只供制作参考；WAV为现有基础合成器，不是真实吉他或钢琴演奏。',
        '自由文本证据按有限关键词提取；未识别内容保留在raw_text/other，不声称完整理解。']
    if re.search(r'比刚才|比之前|更有',text):
        brief.limitations.append('比较性需求需要指定可信参考项目；没有参考时只能作绝对方向建议。')
    unsupported_meter = any((int(m[1]), int(m[2])) != (4,4) for m in re.finditer(r'(\d+)\s*/\s*(\d+)',text))
    if unsupported_meter or re.search(r'分钟|分\s*\d+\s*秒|\d+\s*秒',text):
        raise ValueError('Duration/non-4/4 request needs an explicit supported bar-count revision; not silently shortened')
    return brief


def flatten_spec(spec):
    data = spec.to_dict()
    return {**{k:v for k,v in data.items() if k != 'arrangement'},
...
def enforce_brief(brief, spec):
    ...
    a = spec.arrangement
    if 'avoid_dense' in brief.interpretation_rules and (a.melody_density != 'sparse' or a.drum_density != 'sparse'):
        raise ValueError('Plan violates no-dense interpretation rule')
    if 'avoid_oppressive' in brief.interpretation_rules and (a.harmony != 'resolving' or a.bass_presence != 'low'):
        raise ValueError('Plan violates no-oppressive interpretation rule')
    if ('avoid_loud' in brief.interpretation_rules or 'avoid_oppressive' in brief.interpretation_rules) and a.drum_velocity == 'strong':
...
def creative_result(text, spec_data, planner, reference=None):
    ...
    for name, value in flatten_spec(spec).items():
        if name not in brief.constraints:
            brief.assumptions.append({'parameter': name, 'value': value,
                'source': 'system_inference', 'reason': '系统选择；不是用户明确指定。参考项目参与选择。' if reference else '系统选择；不是用户明确指定。'})
    for quote in brief.explicit['references']:
        brief.reference_interpretations.append({'reference_quote':quote, 'source':'system_inference',
            'features':[f'{spec.arrangement.melody_density}旋律密度', f'{spec.arrangement.vocal_space}人声空间',
...
def mock_creative(text, reference=None):
    brief = extract_brief(text)
    data = CreativeSpec(bpm=80, bars=8, key='A', mode='minor', swing=.12, seed=7).to_dict()
    if reference:
    ...
    if any(x in lower for x in ('克制','留白','留空间','人声','melodic rap','旋律rap')):
        a.update(melody_density='sparse', vocal_space='roomy', drum_density='sparse', bass_presence='low')
    if any(x in lower for x in ('推动力','不甘','推进','drive')):
        a.update(drum_density='moderate', section_variation='lift', melody_variation='moderate', bass_presence='medium', drum_velocity='medium')
    if '吉他' in text:
...
def creative_json_schema():
    ...
    properties['arrangement'] = schema({**{k:{'type':'string','enum':list(v)} for k,v in OPTIONS.items()},
...
def llm_creative(planner, text, reference=None):
    ...
    import json
    brief = extract_brief(text)
    instructions = ('Return a complete original high-level BeatSpec v2, not notes, code, a snapshot or an emotional score. '
        ...
        'Constraints: '+json.dumps(brief.constraints,ensure_ascii=False)+'; rules: '+json.dumps(brief.interpretation_rules)+
        '; trusted reference (may be null): '+json.dumps(reference,ensure_ascii=False))
    result = planner._complete(text, instructions, creative_json_schema())
    try:
        return creative_result(text, result, f'{planner.provider}:{planner.model}', reference)
    except (ValueError, TypeError):
        raise PlannerError('Invalid creative plan or conflict with explicit constraints') from None
