    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def dispatch(self):
            try:
                ...
                is_audio = audio is not None and (
                    self.path in ("/", "/audio") or self.path.startswith("/audio/")
                )
                origin = self.headers.get("Origin")
                if is_audio:
                    expected = f"127.0.0.1:{self.server.server_port}"
                    if self.headers.get("Host") != expected or (
                        origin and origin != "http://" + expected
                    ):
                        return self.send(
                            403,
                            {"error": "Only the loopback same-origin page is allowed"},
                        )
                elif origin:
                    return self.send(
                        403, {"error": "Browser cross-origin requests are disabled"}
                    )
                body = {}
                ...
                if service is None:
                    return self.send(
                        404,
                        {
                            "error": "Use the existing MIDI CLI/API entry for native MIDI"
                        },
                    )
                ...
                if self.command == "POST" and self.path == "/plan":
                    fields(
                        body,
                        ["text", "track_id", "start_bar", "end_bar"],
                        ["text", "track_id", "start_bar", "end_bar"],
                    )
                    return self.send(
                        200, EditPlan.parse(service.planner.plan_edit(**body)).to_dict()
                    )
                ...
                if self.command == "POST" and self.path == "/creative/parse":
                    fields(
                        body,
                        ["text", "reference_project_id", "reference_version"],
                        ["text"],
                    )
                ...
                match = re.fullmatch(
                    r"/projects/([a-f0-9]{32})(?:/versions/([a-f0-9]{32})(?:/(midi|preview))?|/(edits|plan))?",
                    self.path,
                )
                if not match:
                ...
                if self.command == "POST" and edits == "edits":
                    fields(
                        body,
                        ["base_version", "plan", "protected_tracks"],
                        ["base_version", "plan"],
                    )
