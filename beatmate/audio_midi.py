...
def validate_midi(data):
    ...
    if kind not in (0, 1, 2) or not 1 <= tracks <= 256 or (kind == 0 and tracks != 1) or division == 0:
    ...
    for _ in range(tracks):
        ...
        offset += 8 + size
        if offset > len(data): raise AudioError('MIDI轨道不完整')
    if offset != len(data): raise AudioError('MIDI内容长度无效')
        ...
        with zipfile.ZipFile(io.BytesIO(data)) as archive:
            ...
            for member in members:
                ...
                if (path.is_absolute() or '..' in path.parts or '\\' in member.filename or ':' in member.filename
                        or len(member.filename) > 256 or any(ord(c) < 32 for c in member.filename)
                ...
                if path.as_posix().casefold() in names: raise AudioError('MIDI文件包包含重复路径')
                names.add(path.as_posix().casefold())
                if not 0 < member.file_size <= MAX_MIDI_FILE: raise AudioError('MIDI文件大小无效')
                with archive.open(member) as stream: content = stream.read(MAX_MIDI_FILE + 1)
                if len(content) != member.file_size: raise AudioError('MIDI文件不完整')
                validate_midi(content)
                files.append((path.as_posix(), content))
            if not files: raise AudioError('文件包内没有MIDI文件')
            return files
    except (zipfile.BadZipFile, RuntimeError, OSError, ValueError, EOFError, KeyError, IndexError, NotImplementedError):
