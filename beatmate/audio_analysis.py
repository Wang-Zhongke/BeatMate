...
def analysis_config(env, offline=False):
    if offline:
        return dict(
            configured=True,
            provider="mock",
            model="offline-structure-only",
            note="Mock只统计分段与行数，不进行语义分析",
        )
    try:
        ...
        return dict(
            ...
            provider="deepseek",
            model=planner.model,
            note="点击分析会向DeepSeek发送歌词与制作要求，可能产生文本分析费用",
        )
    except ValueError:
        return dict(
            configured=False,
            provider="deepseek",
            model=None,
            note="自动分析需配置DEEPSEEK_API_KEY；也可手动填写编曲摘要",
        )
...
def analyze(raw_text, lyrics, constraints, env, offline=False):
    ...
    try:
        ...
        raise ValueError(
            "自动分析需在后端配置DEEPSEEK_API_KEY和有效模型；也可手动填写编曲摘要"
        ) from None
