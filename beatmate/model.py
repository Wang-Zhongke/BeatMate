...
class BeatSpec:
    ...
    def __post_init__(self):
        if (
            self.style not in ("boom_bap", "trap")
            or self.key not in KEYS
            or self.mode not in ("minor", "major")
        ):
        ...
        if type(self.swing) not in (int, float) or not 0 <= self.swing <= 0.45:
