...
def generate_creative(spec):
    ...
    tracks = [
        dict(
            id=x,
            name=x,
            channel=9 if i < 3 else i - 3,
            program=32 if x == "bass" else 0,
            notes=[],
        )
        for i, x in enumerate(spec.track_ids)
    ]
    by_id = {t["id"]: t for t in tracks}
    scale = SCALES[spec.mode]
    tonic = KEYS.index(spec.key)
    motif = random.Random(f"{spec.seed}:motif:v2").choice(
        ((0, 2, 4, 2, 0), (2, 0, 4, 2, 0), (0, 4, 2, 0, 2))
    )
    ...
    def add(track, bar, tick, pitch, duration, strength):
        by_id[track]["notes"].append(
            note(track, bar, tick, pitch, duration, strength, spec.seed)
        )
    ...
    for bar in range(spec.bars):
        ...
        kick = (
            [0]
            if a.drum_density == "sparse"
            else ([0, 720, 1200] if spec.style == "boom_bap" else [0, 660, 1440])
        )
        snare = [480, 1440] if spec.style == "boom_bap" else [960]
        hat_count = 4 if a.drum_density == "sparse" else 8
        hat_ticks = [
            i * (BAR // hat_count)
            + (round((BAR // hat_count) * spec.swing) if i % 2 else 0)
        ...
        for tick in bass_ticks:
            add(
                ...
                tick,
                tone(chord, 36),
                600 if a.bass_presence == "low" else 840,
                65 if a.bass_presence == "low" else 88,
            )
        ...
        if a.chord_style == "block":
            for pitch in chord_pitches:
                add(
                    ...
                    0,
                    pitch,
                    960 if a.vocal_space == "roomy" else 1680,
                    58,
                )
        else:
            ...
            for i, tick in enumerate(ticks):
                add(
                    "chords", bar, tick, chord_pitches[i % 3], 360, 60 if i == 0 else 53
                )
        if a.melody_enabled:
            positions = (
                (0, 720, 1440)
                if a.melody_density == "sparse"
                else (0, 360, 720, 1080, 1440)
            )
            ...
            for i, tick in enumerate(positions):
                ...
                add(
                    ...
                    tick,
                    pitch,
                    240 if len(positions) > 3 else 360,
                    69 if i == 0 else 61,
                )
    for track in tracks:
...
def thin_notes(notes, start, end):
    ...
    return [
        n for n in notes if not start <= n["start_tick"] < end or n["id"] in keep
    ], removed
...
def reproduce(version):
    ...
    for item in version.get("edit_history", []):
        ...
        if p.operation == "thin_notes":
            ...
            target["notes"] = [
                n for n in target["notes"] if not start <= n["start_tick"] < end
            ]
...
def explain(version):
    ...
    descriptions = {
        ...
        "chord_style": "分解和弦逐音进入。"
        if a["chord_style"] == "arpeggio"
        else "柱式和弦同时进入。",
        "melody_enabled": "生成独立melody轨。"
        if a["melody_enabled"]
        else "未生成melody轨。",
        "melody_density": "基础每小节3个位置。"
        if a["melody_density"] == "sparse"
        else "基础每小节5个位置。",
        "melody_variation": "四小节末改变部分动机音高。"
        if a["melody_variation"] == "moderate"
        else "重复两小节动机，不加额外四小节末变奏。",
        "drum_density": "基础每小节1个kick和4个hihat。"
        if a["drum_density"] == "sparse"
        else "基础每小节3个kick和8个hihat。",
        "drum_velocity": f"鼓基础力度{ {'soft': 68, 'medium': 88, 'strong': 108}[a['drum_velocity']] }；hihat更轻。",
        "section_variation": "每隔四小节增添kick、提高鼓力度并变化句末旋律；乐句末加snare。"
        if a["section_variation"] == "lift"
        else "各段不附加推动型变奏。",
        "bass_presence": "每小节1个较短较轻bass音。"
        if a["bass_presence"] == "low"
        else "每小节2个更长、力度更高的bass音。",
        "vocal_space": "移除旋律最后一拍位置；和弦缩短或减少末拍分解音，给人声留时隙。"
        if a["vocal_space"] == "roomy"
        else "保留末拍旋律与较完整和弦时值。",
    ...
    for name, value in a.items():
        status = (
            ...
            if inactive and name in ("melody_density", "melody_variation")
            else "applied"
        )
        choices.append(
            dict(parameter=name, value=value, status=status, effect=descriptions[name])
        )
    return dict(
        user_expressed=version["creative_brief"]["explicit"],
        applied_choices=choices,
        actual_note_counts=notes,
        scoped_edits=copy.deepcopy(version.get("edit_history", [])),
