...
SPEC_SCHEMA = schema(
    dict(
        style=dict(type="string", enum=["boom_bap", "trap"]),
        bpm=dict(type="integer", minimum=40, maximum=220),
        bars=dict(type="integer", minimum=1, maximum=32),
        key=dict(type="string", enum=list(KEYS)),
        mode=dict(type="string", enum=["minor", "major"]),
        swing=dict(type="number", minimum=0, maximum=0.45),
        seed=dict(type="integer", minimum=0, maximum=2**31 - 1),
    )
)
EDIT_SCHEMA = schema(
    dict(
        operation=dict(
            type="string", enum=["velocity", "transpose", "density", "mute"]
        ),
...
class OpenAIPlanner:
    ...
    def __init__(self, api_key, model, transport=None):
        if not api_key or not model:
            raise ValueError(
                "OPENAI_API_KEY and BEATMATE_MODEL are required for openai mode"
            )
    ...
    def _request(self, payload):
        ...
        try:
            ...
            raise PlannerError(
                "LLM request failed; no project changes committed"
            ) from error
    ...
    def _complete(self, text, instructions, output_schema):
        ...
        try:
            ...
            if (
                len(messages) != 1
                or messages[0].get("status", "completed") != "completed"
            ):
    ...
    def parse_intent(self, text):
        result = self._complete(
            ...
            "No audio generation, tools, code or file operations.",
            SPEC_SCHEMA,
        )
        try:
        ...
        return dict(
            spec=spec.to_dict(),
            planner=f"{self.provider}:{self.model}",
            assumptions=[
                "未指定参数由模型补全；生成前可通过 /parse 检查最终 BeatSpec。"
            ],
        )
    ...
    def plan_edit(self, text, track_id, start_bar, end_bar):
        ...
        result = self._complete(
            ...
            "density is 8/16/32 hits per bar for hihat only; mute value=0. Output operation/value only.",
            EDIT_SCHEMA,
        )
        try:
