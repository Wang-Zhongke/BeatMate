    ...
    try:
        with zipfile.ZipFile(io.BytesIO(data)) as archive:
            ...
            for member in members:
                ...
                if member.is_dir() or path.suffix.lower() != ".wav":
                    continue
                if not 0 < member.file_size <= MAX_AUDIO:
                    raise AudioError("分离音轨超过大小限制")
                name = "/".join(path.parts).lower()
                tokens = set(re.split(r"[^a-z]+", name))
                accompaniment = (
                    bool(tokens & {"instrumental", "accompaniment"})
                    or "伴奏" in name
                    or "no_vocals" in name
                )
                vocals = (
                    bool(tokens & {"vocal", "vocals", "voice"}) or "人声" in name
                ) and "no_vocals" not in name
                if accompaniment == vocals:
                    raise AudioError("分离文件包中音轨名称无法明确识别")
                role = "instrumental" if accompaniment else "vocals"
                if role in tracks:
                    raise AudioError("分离文件包包含重复音轨")
                with archive.open(member) as stream:
                    audio = stream.read(MAX_AUDIO + 1)
                if len(audio) != member.file_size:
                    raise AudioError("分离音轨内容不完整")
                tracks[role] = audio
            if set(tracks) != {"instrumental", "vocals"}:
                raise AudioError("分离文件包缺少伴奏或人声音轨")
            return tracks
