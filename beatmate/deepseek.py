    ...
    def _request(self, payload):
        ...
        try:
            ...
            if status in (401, 403):
                raise PlannerError('DeepSeek authentication/permission failed', code='authentication') from None
            if status == 429:
                raise PlannerError('DeepSeek rate limit reached', code='rate_limit') from None
            if status in (400, 402, 404, 422):
                raise PlannerError('DeepSeek rejected configuration, request or account balance', code='request_rejected') from None
            raise PlannerError('DeepSeek HTTP request failed', code='http_error') from None
        except (urllib.error.URLError, TimeoutError, OSError, http.client.HTTPException):
            raise PlannerError('DeepSeek network unavailable or request timed out', code='network_unavailable') from None
