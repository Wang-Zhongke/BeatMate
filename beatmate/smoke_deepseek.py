...
def run_smoke(*, enabled=False, output='output/deepseek-smoke', environ=None):
    ...
    if not env.get('DEEPSEEK_API_KEY', '').strip():
        return dict(status='SKIPPED', reason='DEEPSEEK_API_KEY is not configured', calls=0)
    calls = 0
    try:
        ...
        if parsed['spec'] != expected_spec:
            raise ValueError('BeatSpec did not preserve the explicit requested parameters')
        plan = planner.plan_edit('将踩镲密度改成每小节16个音符，其他内容不变。', 'hihat', 3, 4)
        ...
        with tempfile.TemporaryDirectory(prefix='beatmate-deepseek-smoke-') as temporary:
            service = Service(Path(temporary) / 'state.sqlite3')
            before = service.create_project(spec=parsed['spec'], protected_tracks=['kick', 'snare', 'bass', 'chords'])
            after = service.edit_project(before['project_id'], before['version_id'], plan)
            if after['parent_id'] != before['version_id'] or after['version_id'] == before['version_id']:
                raise ValueError('Version lineage failed')
            if service.get_version(before['project_id'], before['version_id']) != before:
            ...
            for old, new in zip(before['tracks'], after['tracks']):
                ...
                if old['id'] == 'hihat':
                    if [n for n in old['notes'] if n['start_tick'] < 2*BAR] != [n for n in new['notes'] if n['start_tick'] < 2*BAR]:
            ...
            for name, version in [('before', before), ('after', after)]:
                ...
                if (decoded.type, decoded.ticks_per_beat, len(decoded.tracks)) != (1, 480, 6):
                ...
                for source, track in zip(version['tracks'], decoded.tracks[1:]):
                    ...
                    for event in track:
                        ...
                        if event.type == 'note_on' and event.velocity:
                            ...
                            recovered.append((event.channel, event.note, start, tick-start, velocity))
                    expected = [(source['channel'], n['pitch'], n['start_tick'], n['duration_tick'], n['velocity']) for n in source['notes']]
                    if active or sorted(recovered) != sorted(expected) or tick != 4*BAR:
        ...
        report = dict(status='PASS', calls=calls, provider='deepseek', output=str(destination.resolve()),
                      checks=['BeatSpec', 'EditPlan', 'scope', 'protected_tracks', 'history', 'MIDI_note_roundtrip'])
        (destination / 'report.json').write_text(json.dumps(report, indent=2))
        return report
    except PlannerError as error:
        return dict(status='SKIPPED' if error.code == 'network_unavailable' else 'FAIL',
                    reason=str(error), calls=calls)
    except ValueError as error:
        return dict(status='FAIL', reason=str(error), calls=calls)
    except (OSError, KeyError):
        return dict(status='FAIL', reason='Local smoke validation or artifact output failed', calls=calls)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--run', action='store_true', help='Explicitly allow up to two paid DeepSeek requests')
