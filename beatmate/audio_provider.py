...
def public_target(url, allowed):
    p = urlsplit(url)
    if (p.scheme != 'https' or p.username or p.password or p.fragment or
            p.port not in (None, 443) or not p.hostname or p.hostname not in allowed):
...
def https_request(method, url, headers, body, limit, allowed):
    ...
    try:
        ...
        if method == 'POST' and p.hostname == 'api.mureka.ai' and p.path == '/v1/song/stem':
            conn.sock.settimeout(STEM_RESPONSE_TIMEOUT)
        r = conn.getresponse()
        if not 200 <= r.status < 300:
            if r.status in (400, 401, 402, 403, 404, 422, 429):
                raise Rejected(f'Provider HTTP {r.status}; check API access, model and credits', r.status)
        ...
        if not sending: raise BeforeSubmissionError('tls_certificate') from None
        raise AudioError('TLS failure after request started; response not confirmed') from None
    except TimeoutError:
        if not sending: raise BeforeSubmissionError(stage) from None
        raise AudioError('Network timeout after request started; response not confirmed') from None
    except AudioError:
        if not sending: raise BeforeSubmissionError('target_validation') from None
        raise
    except Exception:
        if not sending: raise BeforeSubmissionError(stage) from None
        raise AudioError('Network failure after request started; response not confirmed') from None
    finally:
...
class MurekaAdapter:
    ...
    def __init__(self, env=None, transport=None):
        ...
        self.hosts = tuple(x.strip() for x in env.get('MUREKA_DOWNLOAD_HOSTS', 'cdn.mureka.ai').split(',') if x.strip())
        self.transport = transport or https_request

    def errors(self):
        result = []
        if not self.key: result.append('MUREKA_API_KEY missing')
        if self.model not in MODELS: result.append('MUREKA_MODEL must explicitly name ' + ', '.join(MODELS))
        if self.base != 'https://api.mureka.ai': result.append('Only official international https://api.mureka.ai is enabled')
        if not self.enabled: result.append('BEATMATE_AUDIO_LIVE=1 required for paid submissions')
        if any(not re.fullmatch(r'[a-zA-Z0-9.-]+', h) for h in self.hosts): result.append('Invalid download host configuration')
        return result

    def request(self, method, path, payload=None):
        if self.errors(): raise AudioError('; '.join(self.errors()))
        raw = self.transport(method, self.base + path,
                             {'Authorization': 'Bearer ' + self.key, 'Content-Type': 'application/json'},
                             json.dumps(payload).encode() if payload is not None else None,
                             1024 * 1024, ('api.mureka.ai',))
        try:
    ...
    def submit(self, task):
        ...
        payload={'model':task['requested_model'],'prompt':task['brief']['final_prompt'],'n':task['n'],'stream':False}
        if song: payload['lyrics']=task['brief']['lyrics']
        return self.request('POST', '/v1/'+('song' if song else 'instrumental')+'/generate', payload)

    def query(self, task):
        kind='song' if task['brief'].get('creation_mode')=='song_stems' else 'instrumental'
        return self.request('GET', '/v1/'+kind+'/query/' + quote(task['provider_task_id'], safe=''))

    def separate(self, choice):
        url=choice.get('url')
        if not isinstance(url,str) or not url: raise BeforeSubmissionError('target_validation')
        try: public_target(url,self.hosts)
        except Exception: raise BeforeSubmissionError('target_validation') from None
        return self.request('POST','/v1/song/stem',{'url':url,'model':'audio-separation-3'})

    def download_midi(self, job):
        from .audio_midi import MAX_MIDI_ZIP
        return self.transport('GET',job['midi_zip_url'],{},None,MAX_MIDI_ZIP,self.hosts)
    ...
    def download(self, choice):
        ...
        if not isinstance(url, str): raise AudioError('Candidate has no downloadable original audio')
...
class MockAudioAdapter:
    ...
    def query(self, task):
        return {'id': task['provider_task_id'], 'status': 'succeeded', 'model': self.model,
                'choices': [{'id': f"{task['id']}-{i}", 'index': i, 'duration': 4000,
                             'url': 'mock://synthetic-test'} for i in range(task['n'])]}
    ...
    def download(self, choice):
        ...
        with wave.open(out, 'wb') as f:
            ...
            f.writeframes(b''.join(struct.pack('<h', int(2200 * math.sin(2 * math.pi * frequency * i / 16000)
                                  * min(1, i / 1600, (64000-i) / 1600))) for i in range(64000)))
        return out.getvalue()

    def separate(self, choice):
        return {'zip_url':'mock://stem-archive','midi_zip_url':'mock://midi-archive','expires_at':0}
    ...
    def download_midi(self, job):
        ...
        with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED) as archive:
            for name,pitch in [('instrumental',60),('vocals',67)]:
                track=b'\x00\x90'+bytes([pitch,80])+b'\x83\x60\x80'+bytes([pitch,0])+b'\x00\xff\x2f\x00'
    ...
    def download_stems(self, job):
        out=io.BytesIO()
        with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED) as archive:
            archive.writestr('instrumental.wav',self.download({'index':job['index']+5}))
...
def create_audio_adapter(env=None):
    ...
    if provider == 'mock': return MockAudioAdapter()
    if provider == 'mureka': return MurekaAdapter(env)
