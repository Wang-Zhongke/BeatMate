...
def main(argv=None):
    ...
    if not args.run or not os.environ.get("MUREKA_API_KEY"):
        print(
            json.dumps(
                {
                    "status": "SKIPPED",
                    "reason": "Requires --run and backend MUREKA_API_KEY; no paid call made",
                }
            )
        )
        return 0
    env = dict(
        ...
        BEATMATE_AUDIO_MAX_SUBMISSIONS="1",
    )
    s = AudioService(args.audio_dir, env=env)
    if s.adapter.errors():
        print(
            json.dumps({"status": "SKIPPED", "reason": "; ".join(s.adapter.errors())})
        )
        return 0
    ...
    try:
        with manifest.open("x") as f:
            json.dump(
                {"request_id": uuid.uuid4().hex, "raw_text": args.text},
                f,
                ensure_ascii=False,
            )
    ...
    while time.monotonic() < until:
        s.tick()
        t = s.get(t["id"])
        if t["status"] in ("ready", "failed", "uncertain"):
            break
        time.sleep(2)
    t = s.get(t["id"])
    status = (
        ...
        if t["status"] == "ready"
            ...
            "SKIPPED"
            if t.get("provider_http_status") in (401, 402, 403, 429)
            else "FAIL"
        )
        if t["status"] == "failed"
        else "PENDING"
    ...
    report = {
        ...
        "human_listening": "PENDING_HUMAN_REVIEW",
        "reason": "Account/model/credits rejection; inspect provider account"
        if t["status"] == "failed"
        else "Re-run with the same audio-dir to query/download only; uncertain will never resend",
        "submission_limit": 1,
        "candidate_count": 1,
    }
    (Path(args.audio_dir) / "report.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2)
    )
    print(json.dumps(report, ensure_ascii=False, indent=2))
    return 0 if status in ("PASS", "SKIPPED", "PENDING") else 1
