...
def midi_bytes(version):
    ...
    meta.extend([mido.MetaMessage('track_name', name='BeatMate tempo'),
    ...
    for source in version['tracks']:
        ...
        track.append(mido.Message('program_change', channel=channel, program=source['program']))
        events = []
        for n in source['notes']:
            events.append((n['start_tick'], 1, mido.Message('note_on', channel=channel, note=n['pitch'], velocity=n['velocity'])))
            events.append((n['start_tick'] + n['duration_tick'], 0, mido.Message('note_off', channel=channel, note=n['pitch'], velocity=0)))
        last = 0
...
def preview_bytes(version, sample_rate=16000):
    ...
    for track in version['tracks']:
        for n in track['notes']:
            ...
            kind = track['id']
            duration = min(duration, .15 if kind == 'hihat' else .25) if kind in ('kick', 'snare', 'hihat') else duration
            count = min(round(duration * sample_rate), length-start)
            frequency = 440 * 2 ** ((n['pitch']-69)/12)
            for i in range(count):
                t = i / sample_rate
                if kind == 'kick':
                    sample = math.sin(2 * math.pi * (48*t + 3*(1-math.exp(-30*t)))) * math.exp(-18*t)
                elif kind in ('snare', 'hihat'):
                    sample = rng.uniform(-1, 1) * math.exp(-(35 if kind == 'hihat' else 20)*t)
                else:
                    envelope = min(1, t/.008) * min(1, (duration-t)/.025)
                    sample = math.sin(2*math.pi*frequency*t) * envelope * (.65 if kind == 'bass' else .3)
                mix[start+i] += sample * n['velocity']/127 * .22
