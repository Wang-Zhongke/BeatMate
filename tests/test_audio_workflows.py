...
class CountingAdapter(MockAudioAdapter):
    ...
    def separate(self, c):
        self.separations += 1
        if self.separation_error:
            raise self.separation_error
...
class WorkflowTests(unittest.TestCase):
    def setUp(self):
        ...
        self.s = AudioService(
            self.tmp.name, self.adapter, {"BEATMATE_AUDIO_MAX_CANDIDATES": "2"}
        )
    ...
    def test_song_lyrics_and_description_have_independent_limits(self):
        ...
        for desc, lyrics in [("描" * 1025, "词"), ("描述", "词" * 5001), ("描述", "")]:
            with self.assertRaises(ValueError):
                prepare(desc, creation_mode="song_stems", lyrics=lyrics)
        with self.assertRaises(ValueError):
            self.s.create("", uuid.uuid4().hex, creation_mode="song_stems", lyrics="词")
        with self.assertRaises(ValueError):
            self.s.create(
                "80 BPM",
                uuid.uuid4().hex,
    ...
    def test_summary_sends_only_reviewed_summary_and_keeps_sources(self):
        ...
        brief = prepare(
            "保持用户原话",
            creation_mode="lyrics_summary",
            lyrics=lyrics,
            arrangement_summary="小调吉他，主歌稀疏，副歌加强鼓组。",
        )
        ...
        with self.assertRaises(ValueError):
            prepare("", creation_mode="lyrics_summary", lyrics=lyrics)
        with self.assertRaises(ValueError):
            prepare(
                "",
                creation_mode="lyrics_summary",
                lyrics=lyrics,
                arrangement_summary="字" * 981,
            )

    def test_analysis_idempotency_and_source_changes(self):
        source = dict(
            raw_text="克制", lyrics="原文\n" * 1500, request_id=uuid.uuid4().hex
        )
        with patch(
            "beatmate.audio.analyze",
            return_value=dict(summary="克制吉他", provider="test", model="test"),
        ) as call:
        ...
        with self.assertRaises(ValueError):
            self.s.preview(
                ...
                creation_mode="lyrics_summary",
                lyrics=source["lyrics"],
                arrangement_summary="摘要",
                analysis_id=first["id"],
            )
        with self.assertRaises(ConflictError):
            self.s.analyze_lyrics(**dict(source, lyrics="新原文"))
    ...
    def test_failed_analysis_never_auto_repeats_or_exposes_secret(self):
        ...
        with patch(
            "beatmate.audio.analyze", side_effect=RuntimeError("secret-key")
        ) as call:
            with self.assertRaises(PlannerError) as error:
                self.s.analyze_lyrics(**source)
            self.assertNotIn("secret-key", str(error.exception))
            with self.assertRaises(ConflictError):
                self.s.analyze_lyrics(**source)
            self.assertEqual(call.call_count, 1)
        with self.s.connect() as c:
            data = c.execute("SELECT data FROM audio_analyses").fetchone()[0]
    ...
    def test_deepseek_receives_full_5000_char_lyrics_without_midi_limit(self):
        ...
        with patch(
            "beatmate.deepseek.DeepSeekPlanner._request", return_value=response
        ) as request:
        ...
        with patch(
            "beatmate.deepseek.DeepSeekPlanner._request",
            return_value=dict(status="completed", output=[{"type": "function_call"}]),
        ):
            with self.assertRaises(PlannerError):
                analyze("", "词", {}, {"DEEPSEEK_API_KEY": "test"})
    ...
    def test_three_tracks_per_candidate_and_restart(self):
        ...
        for index in (0, 1):
            ...
            self.assertEqual(
                {a["track_type"] for a in tracks}, {"mix", "instrumental", "vocals"}
            )
            mix = next(a for a in tracks if a["track_type"] == "mix")
            self.assertTrue(
                all(a["source_audio_asset_id"] == mix["id"] for a in tracks)
            )
            for a in tracks:
                self.assertTrue(
                    self.s.file(a["id"])[0].read_bytes().startswith(b"RIFF")
                )
        self.assertTrue(all(j["status"] == "ready" for j in h["stems"]))
        self.assertNotIn("zip_url", json.dumps(h))
        with self.s.connect() as c:
            self.assertEqual(
                c.execute("SELECT count(*) FROM audio_stem_jobs").fetchone()[0], 2
            )
        other = AudioService(self.tmp.name, self.adapter, {})
        other.tick()
        other.close()
    ...
    def test_zip_download_retry_does_not_resubmit_separation(self):
        ...
        self.finish()
        h = self.s.history()
        self.assertEqual(len(h["assets"]), 1)
        job = h["stems"][0]
        self.assertEqual(job["status"], "downloading")
        self.adapter.archive_error = None
        self.s.retry_stems(job["id"])
        self.s.tick()
    ...
    def test_process_death_after_paid_submit_cannot_resubmit(self):
        ...
        self.adapter.separation_error = None
        self.s.tick()
    ...
    def test_rejected_separation_is_not_retried(self):
        for failure in (
            Rejected("rejected", 402),
            BeforeSubmissionError("tls_certificate"),
        ):
            with self.subTest(failure=failure):
                self.adapter.separation_error = failure
                self.song()
                self.finish()
    ...
    def test_stem_timeout_persists_safe_diagnostics_and_never_resubmits(self):
        self.adapter.separation_error = AudioError(
    ...
    def test_extra_provider_candidate_is_not_separated_without_reservation(self):
        ...
        with patch.object(
            self.adapter, "query", side_effect=lambda t: original(dict(t, n=2))
        ):
            self.finish()
        self.assertEqual(self.adapter.separations, 1)
    ...
    def test_mureka_routes_and_separation_payload(self):
        ...
        task = dict(
            brief=brief, requested_model="mureka-9", n=1, provider_task_id="123"
        )
        with patch.object(adapter, "request", return_value={}) as request:
            ...
            adapter.query(task)
            self.assertEqual(request.call_args.args, ("GET", "/v1/song/query/123"))
            ...
            self.assertEqual(
                ...
                (
                    ...
                    "/v1/song/stem",
                    {
                        "url": "https://cdn.mureka.ai/test.mp3",
        ...
        with (
            patch(
                ...
                side_effect=AudioError("unsafe"),
            ),
            patch.object(adapter, "request") as request,
        ):
            with self.assertRaises(BeforeSubmissionError):
                adapter.separate({"url": "http://127.0.0.1/test.mp3"})
    ...
    def test_http_analysis_and_three_independent_downloads(self):
        ...
        def req(method, path, body=None):
            ...
            conn.close()
            return code, data, disposition

        try:
            source = dict(raw_text="稀疏吉他", lyrics="歌词原文\n" * 700)
            code, analysis, _ = req(
                "POST",
                "/audio/analyze-lyrics",
                dict(source, request_id=uuid.uuid4().hex),
            )
            ...
            self.assertEqual(
                req(
                    ...
                    dict(
                        source,
                        creation_mode="lyrics_summary",
                        arrangement_summary=analysis["summary"],
            ...
            for asset in assets:
                code, data, name = req(
                    "GET", "/audio/assets/" + asset["id"] + "/download"
                )
                self.assertEqual(code, 200)
...
class ArchiveTests(unittest.TestCase):
    ...
    def test_archive_role_mapping_and_rejections(self):
        self.assertEqual(
            set(
                split_archive(
                    self.pack(["tracks/Accompaniment.wav", "tracks/Vocals.wav"])
                )
            ),
            {"instrumental", "vocals"},
        )
        for names in (
            ...
            ["/instrumental.wav", "vocals.wav"],
            ["instrumental.wav"],
            ["instrumental.wav", "vocals.wav", "vocal.wav"],
            ["track1.wav", "track2.wav"],
            ["instrumental.wav", "vocals.wav", "../info.txt"],
        ):
            with self.subTest(names=names), self.assertRaises(AudioError):
                split_archive(self.pack(names))

    def test_zip_symlinks_and_uncompressed_limit(self):
        info = zipfile.ZipInfo("instrumental.wav")
        info.external_attr = (stat.S_IFLNK | 0o777) << 16
        data = io.BytesIO()
        with zipfile.ZipFile(data, "w") as f:
            f.writestr(info, "elsewhere")
        with self.assertRaises(AudioError):
            split_archive(data.getvalue())
        with patch("beatmate.audio_stems.MAX_STEM_TOTAL", 7):
            with self.assertRaises(AudioError):
                split_archive(self.pack(["instrumental.wav", "vocals.wav"]))
