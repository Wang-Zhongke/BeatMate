...
class CoreTests(unittest.TestCase):
    def setUp(self):
        ...
        self.v = self.service.create_project(
            text="90 BPM boom bap 8小节 C minor seed 7",
            protected_tracks=["kick", "bass"],
        )

    def edit(self, plan=None, base=None, **kwargs):
        return self.service.edit_project(
            self.v["project_id"],
            base or self.v["version_id"],
            plan or EditPlan("hihat", 3, 4, "density", 32).to_dict(),
            **kwargs,
        )
    ...
    def test_scope_and_protection_hashes(self):
        ...
        self.assertEqual(
            digest(
                self.service.get_version(self.v["project_id"], self.v["version_id"])
            ),
            old_hash,
        )
    ...
    def test_rejections_leave_database_unchanged(self):
        bad = [
            EditPlan("kick", 1, 1, "mute", 0).to_dict(),
            dict(
                track_id="hihat", start_bar=1, end_bar=9, operation="density", value=32
            ),
            dict(
                track_id="hihat", start_bar=1, end_bar=2, operation="density", value=17
            ),
            dict(
                track_id="snare",
                start_bar=1,
    ...
    def test_concurrent_compare_and_swap(self):
        ...
        with self.service.connect() as conn:
            self.assertEqual(
                conn.execute("SELECT COUNT(*) FROM versions").fetchone()[0], 2
            )
    ...
    def test_midi_roundtrip(self):
        ...
        self.assertEqual(
            (midi.type, midi.ticks_per_beat, len(midi.tracks)), (1, 480, 6)
        )
        self.assertEqual(
            next(m.tempo for m in midi.tracks[0] if m.type == "set_tempo"),
            mido.bpm2tempo(90),
        )
        ...
        for source, track in zip(v["tracks"], midi.tracks[1:]):
            ...
            self.assertEqual(
                sorted(recovered),
                sorted(
                    (n["pitch"], n["start_tick"], n["duration_tick"], n["velocity"])
    ...
    def test_preview(self):
        ...
        with wave.open(io.BytesIO(preview_bytes(v)), "rb") as wav:
            self.assertEqual(
                (wav.getnchannels(), wav.getsampwidth(), wav.getnframes()),
                (1, 2, 32000),
            )
    ...
    def test_melodic_edit_and_mute(self):
        ...
        for a, b in zip(before, after):
            self.assertEqual(
                b["pitch"] - a["pitch"], 12 if BAR <= a["start_tick"] < 2 * BAR else 0
            )
            self.assertEqual(a["id"], b["id"])
        muted = self.edit(
            EditPlan("chords", 2, 2, "mute", 0).to_dict(), result["version_id"]
        )
        self.assertFalse(
            any(BAR <= n["start_tick"] < 2 * BAR for n in muted["tracks"][4]["notes"])
        )
    ...
    def test_generation_extremes(self):
        for style in ("trap", "boom_bap"):
            for swing in (0, 0.45):
                spec = BeatSpec(
                    style=style, bars=32, bpm=40, swing=swing, key="B", seed=2**31 - 1
                )
