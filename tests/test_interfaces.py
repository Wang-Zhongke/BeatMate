...
class HTTPTests(unittest.TestCase):
    ...
    def request(self, method, path, body=None, raw=None, headers=None):
        ...
        try:
            conn.request(method,path,raw if raw is not None else json.dumps(body) if body is not None else None,
    ...
    def test_http_complete_flow(self):
        ...
        code, v=self.request('POST','/projects',{'spec':parsed['spec'],'protected_tracks':['kick']})
        self.assertEqual(code,201)
        pid, vid=v['project_id'],v['version_id']
        code, plan=self.request('POST','/plan',dict(text='加密32',track_id='hihat',start_bar=3,end_bar=4))
        self.assertEqual(code,200)
        code,new=self.request('POST',f'/projects/{pid}/edits',dict(base_version=vid,plan=plan))
        self.assertEqual(code,201)
        self.assertEqual(self.request('POST',f'/projects/{pid}/edits',dict(base_version=vid,plan=plan))[0],409)
        ...
        for suffix, magic in [('midi',b'MThd'),('preview',b'RIFF')]:
            code,data=self.request('GET',f'/projects/{pid}/versions/{new["version_id"]}/{suffix}')
            self.assertEqual(code,200)
    ...
    def test_http_input_errors(self):
        ...
        self.assertEqual(self.request('POST','/parse',{'text':'x'},headers={'Origin':'https://other.example','Content-Type':'application/json'})[0],403)
...
class PlannerTests(unittest.TestCase):
    ...
    def test_llm_spec_and_scope(self):
        ...
        def transport(payload):
            ...
            return self.response(BeatSpec().to_dict() if len(calls)==1 else dict(operation='density',value=32))
        planner=OpenAIPlanner('test-key','user-configured-model',transport)
        self.assertEqual(planner.parse_intent('暗色 boom bap')['spec'],BeatSpec().to_dict())
        plan=planner.plan_edit('加密','hihat',3,4)
        self.assertEqual((plan['track_id'],plan['start_bar'],plan['end_bar']),('hihat',3,4))
    ...
    def test_llm_rejects_invalid_refusal_incomplete_and_expanded_scope(self):
        for response in [dict(status='incomplete'),dict(status='completed',output=[]),
            ...
            planner=OpenAIPlanner('key','model',lambda _, response=response: response)
