...
class DeepSeekRequestTests(OfflineCase):
    def test_request_endpoint_headers_schema_and_system_constraints(self):
        ...
        self.assertEqual(
            request.get_header("Authorization"), "Bearer " + ENV["DEEPSEEK_API_KEY"]
        )
        ...
        self.assertEqual(
            set(fmt["schema"]["required"]), set(BeatSpec.__dataclass_fields__)
        )
        self.assertEqual(parsed["planner"], "deepseek:deepseek-flash")
        self.assertNotIn(
            ENV["DEEPSEEK_API_KEY"], json.dumps(parsed) + json.dumps(payload)
        )
    ...
    def test_custom_model_and_base_url(self):
        planner = DeepSeekPlanner(
            environ={
                **ENV,
                "DEEPSEEK_MODEL": "custom-flash-snapshot",
                "DEEPSEEK_BASE_URL": "https://gateway.example/v1/",
            },
            transport=lambda _: response(BeatSpec().to_dict()),
        )
        self.assertEqual(planner.endpoint, "https://gateway.example/v1/responses")
        self.assertEqual(
            planner.parse_intent("beat")["planner"], "deepseek:custom-flash-snapshot"
        )
    ...
    def test_network_timeout_and_bad_wire_json(self):
        for error in (
            ...
            with (
                patch("urllib.request.build_opener", return_value=opener),
                self.assertRaises(PlannerError) as caught,
            ):
                DeepSeekPlanner(environ=ENV).parse_intent("beat")
        ...
        for raw in (b"", b"{", b"null", b"[]", b"\xff", b"x" * 1_048_577):
            ...
            with (
                patch("urllib.request.build_opener", return_value=opener),
                self.assertRaises(PlannerError),
            ):
                DeepSeekPlanner(environ=ENV).parse_intent("beat")
...
class DeepSeekValidationTests(OfflineCase):
    def setUp(self):
        ...
        self.before = self.service.create_project(
            spec={}, protected_tracks=["kick", "snare", "bass", "chords"]
        )

    def assert_unchanged(self):
        self.assertEqual(
            self.service.get_version(self.before["project_id"]), self.before
        )
        with self.service.connect() as conn:
            self.assertEqual(
                conn.execute("SELECT COUNT(*) FROM versions").fetchone()[0], 1
            )
            self.assertEqual(
    ...
    def test_incomplete_reason_is_safe_and_never_commits(self):
        for reason in ("max_output_tokens", "content_filter", ENV["DEEPSEEK_API_KEY"]):
            ...
            self.service.planner = DeepSeekPlanner(
                environ=ENV, transport=lambda _: candidate
            )
            with self.assertRaises(PlannerError) as caught:
    ...
    def test_malformed_responses_never_create_versions(self):
        variants = [
            ...
            {
                "status": "completed",
                "output": [{"type": "function_call", "name": "execute_python"}],
            },
            {
                "status": "completed",
            ...
            response(text="{"),
            response(text="```json\n{}\n```"),
            response(text='{"bpm":90,"bpm":100}'),
            response(text='{"swing":NaN}'),
            response(text="null"),
            response(text="[]"),
        ]
        ...
        for candidate in variants:
            self.service.planner = DeepSeekPlanner(
                environ=ENV, transport=lambda _, candidate=candidate: candidate
            )
    ...
    def test_strict_spec_fields_and_business_limits(self):
        ...
        variants = [
            {},
            dict(valid, extra="ignored?"),
            dict(valid, bpm=True),
            dict(valid, bpm=221),
            dict(valid, bars=33),
            dict(valid, key="H"),
            dict(valid, mode="dorian"),
            dict(valid, swing=0.9),
            dict(valid, seed=-1),
        ]
    ...
    def test_expanded_scope_unknown_fields_and_operations_rejected(self):
        variants = [
            dict(operation="density", value=16, track_id="kick"),
            dict(operation="density", value=16, start_bar=1),
            dict(operation="density", value=16, end_bar=8),
            dict(operation="density", value=16, protected_tracks=[]),
            dict(operation="run_python", value=0),
            dict(operation="density", value=17),
            dict(operation="velocity", value=127),
            dict(operation="mute", value=True),
            dict(operation="mute", value="0"),
    ...
    def test_end_to_end_scope_history_midi_and_protection(self):
        ...
        after = self.service.edit_project(
            self.before["project_id"], self.before["version_id"], plan
        )
        ...
        for a, b in zip(self.before["tracks"], after["tracks"]):
            if a["id"] != "hihat":
                ...
                outside = lambda t: [
                    n for n in t["notes"] if not 2 * BAR <= n["start_tick"] < 4 * BAR
                ]
        ...
        self.assertEqual(
            digest(
                self.service.get_version(
                    self.before["project_id"], self.before["version_id"]
                )
            ),
            before_hash,
        )
    ...
    def test_executor_rejects_protected_track_and_final_velocity_without_commit(self):
        for track, operation, value in [
            ("kick", "mute", 0),
            ("hihat", "velocity", 126),
        ]:
            plan = self.planner(dict(operation=operation, value=value)).plan_edit(
                "edit", track, 3, 4
            )
            with self.assertRaises(ValueError):
                self.service.edit_project(
        ...
        plan = self.planner(dict(operation="mute", value=0)).plan_edit(
            "edit", "hihat", 3, 4
        )
        with self.assertRaises(ValueError):
            self.service.edit_project(
                self.before["project_id"],
                self.before["version_id"],
                plan,
                protected_tracks=["hihat"],
            )
        self.assert_unchanged()
...
class ConfigurationTests(OfflineCase):
    def test_default_mock_and_configuration_isolation(self):
        ...
        planner = create_planner(
            ...
            environ={
                "OPENAI_API_KEY": "openai-test-only",
                "BEATMATE_MODEL": "configured-model",
                "DEEPSEEK_API_KEY": "",
            },
        )
    ...
    def test_explicit_mock_overrides_bad_environment_and_unknown_fails(self):
        self.assertIsInstance(
            create_planner("mock", environ={"BEATMATE_PLANNER": "invalid"}), MockPlanner
        )
        for name in ("", "invalid"):
            with self.assertRaises(ValueError):
    ...
    def test_cli_environment_selection_and_override(self):
        ...
        for argument, env, expected in [
            ([], {"BEATMATE_PLANNER": "deepseek", **ENV}, DeepSeekPlanner),
            ...
            with (
                ...
                patch(
                    ...
                    [
                        "beatmate",
                        "--db",
                        str(Path(self.tmp.name) / "cli.db"),
                        "serve",
                        *argument,
                    ],
                ),
                patch("beatmate.cli.server") as serve,
    ...
    def test_cli_invalid_provider_fails_before_database_creation(self):
        ...
        with (
            patch.dict("os.environ", {"BEATMATE_PLANNER": "invalid"}, clear=True),
            patch("sys.argv", ["beatmate", "--db", str(path), "serve"]),
    ...
    def test_demo_remains_offline_with_deepseek_environment(self):
        ...
        with (
            patch.dict("os.environ", {"BEATMATE_PLANNER": "deepseek"}, clear=True),
            patch(
                "sys.argv",
                [
                    "beatmate",
                    "--db",
                    str(Path(self.tmp.name) / "demo.db"),
                    "demo",
                    "--output",
                    str(output),
                ],
            ...
            main()
        self.assertEqual(
            json.loads((output / "v1.json").read_text())["audit"]["planner"], "mock"
        )
...
class SmokeTests(OfflineCase):
    def test_smoke_default_and_missing_key_skip_without_network(self):
        with patch(
            "beatmate.smoke_deepseek.DeepSeekPlanner",
    ...
    def test_smoke_two_injected_calls_complete_real_local_execution(self):
        responses = [
            response(BeatSpec(bars=4, seed=7).to_dict()),
            response(dict(operation="density", value=16)),
        ]
        with patch.object(
            DeepSeekPlanner, "_request", side_effect=responses
        ) as request:
            result = run_smoke(enabled=True, environ=ENV, output=self.tmp.name)
        self.assertEqual(
            (result["status"], result["calls"], request.call_count), ("PASS", 2, 2)
        )
        self.assertTrue((Path(result["output"]) / "after.mid").is_file())
        self.assertNotIn(
            ENV["DEEPSEEK_API_KEY"],
            (Path(result["output"]) / "report.json").read_text(),
        )

    def test_smoke_network_skip_and_auth_failure(self):
        for code, status in [
            ("network_unavailable", "SKIPPED"),
            ("authentication", "FAIL"),
        ]:
            with patch.object(
                DeepSeekPlanner,
                "_request",
                side_effect=PlannerError("Safe error", code=code),
            ) as request:
                result = run_smoke(enabled=True, environ=ENV, output=self.tmp.name)
            self.assertEqual(
                (result["status"], result["calls"], request.call_count), (status, 1, 1)
            )
    ...
    def test_smoke_wrong_semantics_fails_not_200_success(self):
        with patch.object(
            DeepSeekPlanner,
            "_request",
            return_value=response(BeatSpec(bars=8).to_dict()),
        ) as request:
            result = run_smoke(enabled=True, environ=ENV, output=self.tmp.name)
...
class DeepSeekHTTPTests(OfflineCase):
    def test_http_provider_failure_is_502_and_no_version(self):
        planner = DeepSeekPlanner(
            environ=ENV, transport=lambda _: response({"unexpected": "output"})
        )
        ...
        try:
            for path, body in [
                ("/projects", dict(text="beat")),
                ("/plan", dict(text="edit", track_id="hihat", start_bar=3, end_bar=4)),
            ]:
