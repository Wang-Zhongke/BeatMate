...
test('late responses cannot replace the newest search', async () => {
  ...
  const load = createCatalogLoader(
    (q) => new Promise((resolve) => waiting.set(q.search, resolve)),
    (p) => applied.push(p),
  );
  const old = load({ search: 'old' }),
    latest = load({ search: 'new' });
  await Promise.resolve();
  waiting.get('new')('new results');
  await latest;
  waiting.get('old')('old results');
  await old;
...
test('polling coalesces identical in-flight page requests and recovers after errors', async () => {
  ...
  const a = load({ offset: 0 }),
    b = load({ offset: 0 });
  await Promise.resolve();
  resolve({});
  await Promise.all([a, b]);
  assert.equal(calls, 1);
  const c = load({ offset: 0 });
  await Promise.resolve();
  resolve({});
  await c;
  assert.equal(calls, 2);
  let fail = true;
  const recover = createCatalogLoader(
    () => {
  ...
  await assert.rejects(recover({}), /offline/);
  fail = false;
  await recover({});
});
test('a failed obsolete search does not surface an error over the current page', async () => {
  ...
  const applied = [];
  const load = createCatalogLoader(
    (q) => (q.search === 'old' ? new Promise((_, reject) => (rejectOld = reject)) : 'current'),
    (p) => applied.push(p),
  );
  const old = load({ search: 'old' });
  await Promise.resolve();
  await load({ search: 'new' });
