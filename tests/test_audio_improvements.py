...
class ImprovementsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.s = AudioService(self.tmp.name, env={})
        self.addCleanup(self.s.close)

    def generate(self, song=True):
        kw = (
            dict(creation_mode="song_stems", lyrics="测试歌词", reviewed=True)
            if song
            else {}
        )
        ...
        for _ in range(4):
            self.s.tick()
        h = self.s.history()
        return next(a for a in h["assets"] if a["source_audio_asset_id"] == a["id"])
    ...
    def test_health_is_local_and_detects_source_and_configuration_changes(self):
        ...
        p = Path(self.tmp.name) / "config"
        p.write_text("MUREKA_API_KEY=private-test")
    ...
    def test_network_diagnostic_never_sends_http_or_credentials(self):
        ...
        with (
            patch(
                "beatmate.audio_health.public_target",
                return_value=(MagicMock(hostname="api.mureka.ai"), "8.8.8.8"),
            ),
            patch("beatmate.audio_health.socket.create_connection", return_value=raw),
            patch(
                "beatmate.audio_health.ssl.create_default_context", return_value=context
            ),
            patch("http.client.HTTPSConnection.request") as request,
        ):
            report = self.s.health.network(self.s)
            self.assertEqual(report["status"], "ok")
            self.assertFalse(report["http_sent"])
            request.assert_not_called()
            context.wrap_socket.assert_called_once_with(
                raw, server_hostname="api.mureka.ai"
            )
            secured.close.assert_called_once()
        with patch(
            "beatmate.audio_health.public_target", side_effect=OSError("private-test")
        ):
    ...
    def test_progress_does_not_offer_resubmission_for_uncertain(self):
        ...
        self.s.tick()
        self.assertIsNotNone(self.s.get(t["id"])["progress"]["last_attempt_at"])

    def test_notes_survive_restart_and_validate_scope(self):
        a = self.generate()
        aid = a["id"]
        notes = self.s.update_note(aid, "add", 1.2, "副歌前留白")
        again = AudioService(self.tmp.name, env={})
        self.addCleanup(again.close)
        self.assertEqual(again.notes(aid), notes)
        for seconds in (-1, 100, float("nan"), True):
            with self.assertRaises(ValueError):
                self.s.update_note(aid, "add", seconds, "test")
        with self.assertRaises(ValueError):
            self.s.update_note(aid, "add", 1, " " * 3)
        other = next(x for x in self.s.history()["assets"] if x["id"] != aid)
        with self.assertRaises(KeyError):
            self.s.update_note(other["id"], "delete", note_id=notes[0]["id"])
        self.assertEqual(len(self.s.notes(aid)), 1)
        self.s.library_update([aid], "trash")
        self.s.library_update([aid], "restore")
        self.assertEqual(len(self.s.notes(aid)), 1)
        self.s.update_note(aid, "delete", note_id=notes[0]["id"])
        self.assertEqual(self.s.notes(aid), [])

    def test_bundle_has_original_tracks_midi_notes_and_safe_names_without_network(self):
        a = self.generate()
        self.s.update_note(a["id"], "add", 2, "调整鼓组")
        before = self.s.budget()
        with (
            patch.object(
                self.s.adapter, "download", side_effect=AssertionError("network")
            ),
            patch.object(
                self.s.adapter, "separate", side_effect=AssertionError("paid")
            ),
        ):
            with self.s.export_bundle(a["id"]) as (f, name):
                data = f.read()
        self.assertNotIn("/", name)
        with zipfile.ZipFile(io.BytesIO(data)) as z:
            names = z.namelist()
            self.assertFalse(any(".." in n.split("/") for n in names))
            manifest = json.loads(
                z.read(next(n for n in names if n.endswith("/manifest.json")))
            )
            self.assertEqual(
                {f["kind"] for f in manifest["files"]},
                {"mix", "instrumental", "vocals", "midi", "midi_archive"},
            )
            self.assertEqual(manifest["notes"][0]["text"], "调整鼓组")
            ...
            for file in manifest["files"]:
                ...
                self.assertEqual(
                    hashlib.sha256(
                        z.read(names[0].split("/")[0] + "/" + file["path"])
                    ).hexdigest(),
                    file["sha256"],
                )
    ...
    def test_partial_export_explains_missing_audio_and_purge_blocks_notes_export(self):
        a = self.generate()
        stem = next(
            x for x in self.s.history()["assets"] if x["track_type"] == "vocals"
        )
        ...
        with self.s.export_bundle(a["id"]) as (f, _):
            with zipfile.ZipFile(f) as z:
                manifest = json.loads(
                    z.read(
                        next(n for n in z.namelist() if n.endswith("/manifest.json"))
                    )
                )
        self.assertIn("vocals", [x["kind"] for x in manifest["unavailable"]])
        self.s.update_note(a["id"], "add", 1, "test")
        self.s.library_update([a["id"]], "trash")
        self.s.library_update([a["id"]], "purge")
        with self.assertRaises(AudioError):
            with self.s.export_bundle(a["id"]):
                pass
        with self.assertRaises(AudioError):
            self.s.notes(a["id"])
        with self.s.connect() as c:
            self.assertEqual(
                c.execute("SELECT COUNT(*) FROM audio_notes").fetchone()[0], 0
            )

    def test_cached_history_rechecks_changed_files_and_download_always_validates(self):
        a = self.generate(song=False)
        path, _ = self.s.file(a["id"])
        self.s.history()
        with patch(
            "beatmate.audio.sha", side_effect=AssertionError("unnecessary read")
        ):
    ...
    def test_port_conflict_reports_actionable_error_without_worker_start(self):
        ...
        try:
            with (
                patch("beatmate.audio_cli.load_audio_env", return_value={}),
                patch("beatmate.audio.AudioService.start") as start,
            ):
