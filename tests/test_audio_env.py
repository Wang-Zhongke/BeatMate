...
class AudioEnvTests(unittest.TestCase):
    ...
    def test_launcher_asks_once_and_reuses_saved_key(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            p = Path(root) / ".env"
            p.write_text("BEATMATE_AUDIO_PROVIDER=mureka\nMUREKA_API_KEY=\n")
            old = os.getcwd()
            try:
                os.chdir(root)
                with (
                    ...
                    patch("getpass.getpass", return_value="test-key") as ask,
                    patch("beatmate.cli.main") as start,
                    ...
                    self.assertEqual(
                        load_audio_env(p, {})["MUREKA_API_KEY"], "test-key"
                    )
                    self.assertNotIn("MUREKA_API_KEY", os.environ)
                with (
                    patch.dict(os.environ, {}, clear=True),
                    patch("getpass.getpass") as ask,
                    patch("beatmate.cli.main") as start,
                    patch("sys.argv", []),
                ):
                    main()
                ...
                with (
                    patch.dict(os.environ, {"MUREKA_API_KEY": ""}, clear=True),
                    patch("sys.stdin.isatty", return_value=True),
                    patch("getpass.getpass", return_value="replacement-key"),
                    patch(
                        "beatmate.cli.main",
                        side_effect=lambda: self.assertEqual(
                            os.environ["MUREKA_API_KEY"], "replacement-key"
                        ),
                    ),
                    patch("sys.argv", []),
                    patch("builtins.print"),
                    ...
                    self.assertEqual(
                        load_audio_env(p, {})["MUREKA_API_KEY"], "replacement-key"
                    )
            finally:
                os.chdir(old)
    ...
    def test_parser_errors_do_not_expose_secret_values(self):
        with tempfile.TemporaryDirectory() as root:
            p = Path(root) / ".env"
            p.write_text("MUREKA_API_KEY='secret-no-close")
