...
from beatmate.creative import Arrangement, CreativeSpec, CreativeEditPlan, mock_creative, creative_result, GENERATOR_VERSION
...
from beatmate.model import BeatSpec, EditPlan, BAR, KEYS, TRACK_IDS, digest, validate_tracks
...
def llm_response(data):
    return dict(status='completed',output=[dict(type='message',content=[dict(type='output_text',text=json.dumps(data))])])
...
class CreativeCase(unittest.TestCase):
    def setUp(self):
        ...
        for target in ('urllib.request.urlopen','urllib.request.OpenerDirector.open'):
            guard=patch(target,side_effect=AssertionError('Offline suite cannot use provider network'))
            guard.start()
    ...
    def create_a(self):
        return self.service.create_creative_project(A_TEXT,protected_tracks=list(TRACK_IDS))


class BriefTests(CreativeCase):
    def test_explicit_provenance_and_assumptions(self):
        result=mock_creative('凌晨一个人回家，想念一个人，XXXTentacion类型的beat。76 BPM，8小节，D小调，seed 12。')
        brief=result['brief']
        ...
        self.assertTrue(all(x['source']=='system_inference' for x in brief['assumptions']))
    ...
    def test_negative_constraints_change_notes(self):
        ...
        self.assertEqual((a['melody_density'],a['drum_density'],a['harmony'],a['bass_presence'],a['drum_velocity']),
                         ('sparse','sparse','resolving','low','soft'))
        self.assertNotEqual(generate_creative(CreativeSpec.parse(dense['spec'])),generate_creative(CreativeSpec.parse(sparse['spec'])))
        self.assertTrue(sparse['brief']['explicit']['prohibitions'])

    def test_conflicting_constraints_not_silently_repaired(self):
        for text in ('80 BPM，90 BPM','0小节','3/4拍','约2分30秒','不要太密，旋律适中','不要分解和弦'):
    ...
    def test_no_fixed_artist_template(self):
        ...
        self.assertNotEqual(generate_creative(CreativeSpec.parse(a['spec'])),generate_creative(CreativeSpec.parse(b['spec'])))
    ...
    def test_relative_creation_uses_trusted_reference(self):
        a=self.create_a()
        result=self.service.parse_creative('带一点不甘，比刚才更有推动力，但不要很吵。',a['project_id'],a['version_id'])
        self.assertEqual(result['spec']['bpm'],76)
        ...
        self.assertTrue(all('参考项目' in x['reason'] for x in result['brief']['assumptions']))
        self.assertEqual(self.count(),1)

    def test_llm_constraints_and_schema_no_snapshots(self):
        calls=[]
        expected=mock_creative('76 BPM，8小节，不要太密')['spec']
        planner=DeepSeekPlanner(environ={'DEEPSEEK_API_KEY':'test-only'},transport=lambda payload:(calls.append(payload) or llm_response(expected)))
        ...
        self.assertNotIn('creative_brief',calls[0]['text']['format']['schema']['properties'])
        self.assertNotIn('tools',calls[0])
        self.assertNotIn('test-only',json.dumps(parsed))
        for bad in [dict(expected,bpm=90),dict(expected,assumptions=['user wants 90']),dict(expected,tracks=[])]:
            self.service.planner=DeepSeekPlanner(environ={'DEEPSEEK_API_KEY':'test-only'},transport=lambda _,bad=bad:llm_response(bad))
            with self.assertRaises(PlannerError):
                self.service.create_creative_project('76 BPM，8小节，不要太密')
        self.assertEqual(self.count(),0)

    def test_llm_does_not_override_prohibitions(self):
        data=CreativeSpec(arrangement=Arrangement(drum_velocity='strong',drum_density='moderate')).to_dict()
...
class GeneratorTests(CreativeCase):
    def test_every_active_field_changes_real_structure(self):
        baseline=CreativeSpec(bars=8,seed=99,arrangement=Arrangement(vocal_space='balanced',chord_style='block'))
        original=generate_creative(baseline)
        cases={'harmony':('descending',{'chords','bass','melody'}),
               'chord_style':('arpeggio',{'chords'}),'melody_density':('sparse',{'melody'}),
               'melody_variation':('moderate',{'melody'}),'drum_density':('moderate',{'kick','snare','hihat'}),
               'drum_velocity':('strong',{'kick','snare','hihat'}),'section_variation':('lift',{'kick','snare','hihat','melody'}),
               'bass_presence':('medium',{'bass'}),'vocal_space':('roomy',{'chords','melody'})}
        for field,(value,targets) in cases.items():
            with self.subTest(field=field):
                changed=generate_creative(replace(baseline,arrangement=replace(baseline.arrangement,**{field:value})))
                actual={a['id'] for a,b in zip(original,changed) if a!=b}
                self.assertTrue(actual)
                self.assertTrue(actual<=targets,(field,actual))
                if field in ('melody_density','drum_density','bass_presence','vocal_space'):
                    self.assertTrue(any(len(a['notes'])!=len(b['notes']) for a,b in zip(original,changed)))
                if field=='drum_velocity':
                    self.assertEqual([[{k:v for k,v in n.items() if k!='velocity'} for n in t['notes']] for t in original],
                                     [[{k:v for k,v in n.items() if k!='velocity'} for n in t['notes']] for t in changed])

    def test_advice_only_timbre_does_not_claim_rendering(self):
        spec=CreativeSpec()
        a=generate_creative(spec)
        b=generate_creative(replace(spec,arrangement=replace(spec.arrangement,timbre_hint='nylon_guitar')))
        self.assertEqual(a,b)
        v=self.service.create_creative_project('吉他，伤感')
        hint=next(x for x in v['explanation']['applied_choices'] if x['parameter']=='timbre_hint')
    ...
    def test_seed_reproduction_motif_scale_range_and_optional_track(self):
        ...
        shape=lambda bar:[(n['pitch'],n['start_tick']%BAR,n['duration_tick']) for n in melody if n['start_tick']//BAR==bar]
        ...
        for bar in range(s.bars):
            ...
            chord_tones={(KEYS.index(s.key)+SCALES[s.mode][(degree+i)%7])%12 for i in (0,2,4)}
    ...
    def test_extreme_specs_valid(self):
        for key in KEYS:
            for mode in ('major','minor'):
                s=CreativeSpec(bars=32,bpm=220,key=key,mode=mode,swing=.45,arrangement=Arrangement(
                    drum_density='moderate',drum_velocity='strong',section_variation='lift',melody_variation='moderate'))
    ...
    def test_six_track_midi_roundtrip_and_reference_wav(self):
        ...
        self.assertEqual((midi.type,midi.ticks_per_beat,len(midi.tracks)),(1,480,7))
        ...
        for track,source in zip(midi.tracks[1:],v['tracks']):
            ...
            self.assertEqual(sorted(notes),sorted((source['channel'],n['pitch'],n['start_tick'],n['duration_tick'],n['velocity']) for n in source['notes']))
            self.assertEqual(tick,2*BAR)
        with wave.open(io.BytesIO(preview_bytes(v)),'rb') as wav:
            self.assertAlmostEqual(wav.getnframes()/wav.getframerate(),2*4*60/76,places=4)
...
class CreativeEditingTests(CreativeCase):
    def test_thinning_scope_anchors_protection_and_reproduction(self):
        ...
        self.assertEqual(proposal['plan'],CreativeEditPlan('melody',9,16,'thin_notes',50).to_dict())
        after=self.service.edit_project(before['project_id'],proposal['base_version'],proposal['plan'],proposal['protected_tracks'])
        for a,b in zip(before['tracks'],after['tracks']):
            if a['id']!='melody':
                self.assertEqual(a,b)
            else:
                self.assertLess(len(b['notes']),len(a['notes']))
                self.assertEqual([n for n in a['notes'] if n['start_tick']<8*BAR],[n for n in b['notes'] if n['start_tick']<8*BAR])
        ...
        self.assertEqual(self.service.get_version(before['project_id'],before['version_id']),before)
        self.assertTrue(all(h['before']==h['after'] for h in after['audit']['protected_hashes'].values()))
        self.assertEqual(self.count(),2)
        with self.assertRaises(ConflictError):
            self.service.edit_project(before['project_id'],before['version_id'],proposal['plan'])
    ...
    def test_explicit_scope_wins_and_missing_scope_not_completed(self):
        v=self.create_a()
        reordered=self.service.plan_project_edit(v['project_id'],'鼓和贝斯不要动，只让后半段旋律更克制。')
        self.assertEqual(reordered['status'],'ready')
        result=self.service.plan_project_edit(v['project_id'],'只让后半段旋律更克制',track_id='melody',start_bar=3,end_bar=4)
        self.assertEqual((result['plan']['start_bar'],result['plan']['end_bar']),(3,4))
        self.assertEqual(result['scope_source'],'caller')
        result=self.service.plan_project_edit(v['project_id'],C_TEXT,track_id='melody')
        self.assertEqual(result['status'],'suggestion')

    def test_missing_protected_invalid_and_boundary_fail_without_version(self):
        v=self.create_a()
        plans=[CreativeEditPlan('melody',1,17,'thin_notes',50).to_dict(),
               dict(track_id='hihat',start_bar=1,end_bar=2,operation='thin_notes',value=50),
               dict(track_id='melody',start_bar=1,end_bar=2,operation='thin_notes',value=30)]
        for plan in plans:
            with self.assertRaises(ValueError):
                self.service.edit_project(v['project_id'],v['version_id'],plan)
        plan=CreativeEditPlan('melody',2,2,'thin_notes',50).to_dict()
        with self.assertRaises(ValueError):
            self.service.edit_project(v['project_id'],v['version_id'],plan,protected_tracks=['melody'])
        crossing=copy.deepcopy(v)
        crossing['tracks'][-1]['notes'][0]['duration_tick']=BAR+60
        with patch.object(self.service,'get_version',return_value=crossing),self.assertRaisesRegex(ValueError,'boundary-crossing'):
            self.service.edit_project(v['project_id'],v['version_id'],plan)
        self.assertEqual(self.count(),1)
        disabled=self.service.create_creative_project('不要主旋律，8小节')
        with self.assertRaises(ValueError):
            self.service.edit_project(disabled['project_id'],disabled['version_id'],plan)
    ...
    def test_melody_legacy_operations_and_density_restriction(self):
        ...
        for operation,value in [('velocity',-4),('transpose',2),('mute',0)]:
            v=self.service.edit_project(v['project_id'],v['version_id'],CreativeEditPlan('melody',1,1,operation,value).to_dict())
            self.assertEqual(reproduce(v),v['tracks'])
        with self.assertRaises(ValueError):
            CreativeEditPlan('melody',1,2,'density',8)
        self.assertEqual([n for n in v['tracks'][-1]['notes'] if n['start_tick']<BAR],[])

    def test_trusted_llm_scope_protection_and_no_operation_substitution(self):
        v=self.create_a(); calls=[]
        self.service.planner=DeepSeekPlanner(environ={'DEEPSEEK_API_KEY':'test-only'},transport=lambda payload:(calls.append(payload) or llm_response(dict(operation='thin_notes',value=50))))
        result=self.service.plan_project_edit(v['project_id'],C_TEXT,track_id='melody',start_bar=2,end_bar=3)
        ...
        for bad in [dict(operation='thin_notes',value=50,track_id='kick'),dict(operation='transpose',value=1),dict(operation='velocity',value=-20)]:
            self.service.planner=DeepSeekPlanner(environ={'DEEPSEEK_API_KEY':'test-only'},transport=lambda _,bad=bad:llm_response(bad))
            with self.assertRaises(PlannerError):
...
class CompatibilityTests(CreativeCase):
    def test_checked_in_legacy_snapshots_and_exports_unchanged(self):
        for name in ('v1','v2'):
            source=Path('examples/demo')/f'{name}.json'
            ...
            with self.service.connect() as c:
                c.execute('INSERT OR IGNORE INTO projects VALUES (?,?)',(v['project_id'],v['version_id']))
                c.execute('INSERT INTO versions VALUES (?,?,?)',(v['version_id'],v['project_id'],original.decode()))
    ...
    def test_old_data_stays_raw_in_database_after_v2_write(self):
        old=self.service.create_project(spec={})
        with self.service.connect() as c:
            raw=c.execute('SELECT data FROM versions WHERE id=?',(old['version_id'],)).fetchone()[0]
        self.create_a()
        with self.service.connect() as c:
            self.assertEqual(c.execute('SELECT data FROM versions WHERE id=?',(old['version_id'],)).fetchone()[0],raw)
    ...
    def test_demo_complete_artifacts_and_human_review_pending(self):
        ...
        for name in ('A','B','C'):
            for file in ('input.txt','creative_brief.json','spec.json','explanation.json','version.json','beat.mid','preview.wav'):
                self.assertTrue((out/name/file).is_file())
        a=json.loads((out/'A/version.json').read_text());b=json.loads((out/'B/version.json').read_text());c=json.loads((out/'C/version.json').read_text())
        self.assertNotEqual(a['tracks'],b['tracks'])
        self.assertEqual(c['parent_id'],a['version_id'])
        self.assertEqual(reproduce(c),c['tracks'])
        self.assertTrue(all(t['outside_unchanged'] for t in json.loads((out/'C/diff.json').read_text())['tracks']))
        self.assertEqual(json.loads((out/'listening-review.json').read_text())['status'],'PENDING_HUMAN_REVIEW')
        with self.assertRaises(ValueError):
            generate_demos(out)
...
class CreativeHTTPTests(CreativeCase):
    def test_parse_create_plan_commit_export_and_reject_client_snapshot(self):
        httpd=server(self.service,0);thread=threading.Thread(target=httpd.serve_forever,daemon=True);thread.start()
        ...
        try:
            code,parsed=request('POST','/creative/parse',dict(text='克制，不要太密，4小节'))
            self.assertEqual(code,200)
            code,v=request('POST','/creative/projects',dict(text='克制，不要太密，4小节',spec=parsed['spec'],protected_tracks=list(TRACK_IDS)))
            self.assertEqual(code,201)
            pid=v['project_id']
            self.assertEqual(request('POST',f'/projects/{pid}/plan',dict(text=C_TEXT,project_state={'bars':99}))[0],400)
            code,p=request('POST',f'/projects/{pid}/plan',dict(text=C_TEXT))
            self.assertEqual(code,200)
            code,after=request('POST',f'/projects/{pid}/edits',{k:p[k] for k in ('base_version','plan','protected_tracks')})
            self.assertEqual(code,201)
            self.assertEqual(request('GET',f'/projects/{pid}/versions/{v["version_id"]}')[1],v)
            for suffix,magic in [('midi',b'MThd'),('preview',b'RIFF')]:
                code,data=request('GET',f'/projects/{pid}/versions/{after["version_id"]}/{suffix}')
                self.assertEqual(code,200);self.assertTrue(data.startswith(magic))
            self.assertEqual(request('POST',f'/projects/{pid}/plan',dict(text='更温暖'))[1]['status'],'suggestion')
        finally:
