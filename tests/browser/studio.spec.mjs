...
test('preview does not create music and repeated confirmation creates one task', async ({
  page,
}) => {
  ...
  let submissions = 0;
  page.on('request', (r) => {
    if (r.method() === 'POST' && r.url().endsWith('/audio/tasks')) submissions++;
  });
...
test('stems, A/B loop, persistent notes, and existing-file export', async ({ page }) => {
  ...
  await page.locator('#loop-start').fill('1');
  ...
  await page.locator('#note-text').fill('自动回归：检查这一段');
  ...
  await expect(page.locator('#works .work-title')).toHaveText(['测试 Beat 00']);
  await page.locator('#works').getByRole('button', { name: '音轨', exact: true }).click();
  await page.getByRole('button', { name: '人声 · WAV', exact: true }).click();
  await page.locator('#listening-open').click();
  await expect(page.locator('#listening-notes')).toContainText('自动回归：检查这一段');
  await page.getByRole('button', { name: '关闭试听工具', exact: true }).click();
  const download = page.waitForEvent('download');
  await page.getByRole('link', { name: '打包导出已保存文件', exact: true }).click();
